...
import ru.enzhine.frame.auth.server.coding.StatefulByteCoder
...
class TokenCoderImpl(
    ...
    private fun codeArray(array: ByteArray): String =
        b64urlEncoded(statefulByteCoder.encode(array))

    private fun codeString(string: String): String =
        b64urlEncoded(statefulByteCoder.encode(string.toByteArray(Charsets.UTF_8)))

    private fun decodeString(string: String): ByteArray =
        statefulByteCoder.decode(b64urlDecoded(string))
    ...
    fun b64urlDecoded(inp: String): ByteArray = inp
        .replace('.', '=')
        .replace('_', '/')
        .replace('-', '+')
